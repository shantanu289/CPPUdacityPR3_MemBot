                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            std::unique_ptr<GraphEdge> edge(new GraphEdge(id)); // or std::make_unique<GraphEdge>(id)
                            edge.get()->SetChildNode((*childNode).get()); // *childNode is the unique pointer. Do a .get() on it to retrieve the raw pointer
                            edge.get()->SetParentNode((*parentNode).get()); //edge itself is now a unique pointer instead of a normal pointer
